...
import { MarsRoverclass } from "../infraestructure/mars-rover-class.enum.ts";
import { IApiResponse } from "../../../models/api-response.model.ts";

export class MarsRoversController extends BaseController {

    constructor() {
        super();
    }

    public async getRoverByCuriosity({ params, response }: IHttpRequestArgs<RoverCameraArgs>): Promise<void> {
        const marsDataService = new MarsDataService();
        const result: IApiResponse<IMarsRoverInfo | undefined> = await marsDataService.getRoverPhotos(MarsRoverclass.CURIOSITY, params);
    ...
    public async getRoverByOpportunity({ params, response }: IHttpRequestArgs<RoverCameraArgs>): Promise<void> {
        const marsDataService = new MarsDataService();
        const result: IApiResponse<IMarsRoverInfo | undefined> = await marsDataService.getRoverPhotos(MarsRoverclass.OPPORTUNITY);
    ...
    public async getRoverBySpirit({ params, response }: IHttpRequestArgs<RoverCameraArgs>): Promise<void> {
        const marsDataService = new MarsDataService();
        const result: IApiResponse<IMarsRoverInfo | undefined> = await marsDataService.getRoverPhotos(MarsRoverclass.SPIRIT);
    ...
    public async addPhotoByRover({ request, response }: { request: Request; response: Response }): Promise<void> {
        if (!request.hasBody) {
            response.status = 400;
        } else {
